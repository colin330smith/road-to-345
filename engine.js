...
const SECONDARY_SLOT = {
  ...
  latwidth: (v) => sx("Unilateral Cable Pulldown", 70, [10, 12, 15], 2, "8", "pulldown", "pulldown", false, "", 10, v),
...
function balanceSlot(framePrimary, v) {
  if (framePrimary === "upperback" || framePrimary === "traps") return sx("Cable Lateral Raise", 20, [15, 20, 25], 2, "8–9", "lateral", "latwed", false, "Balance: side delt", 5, v);
...
const DETAIL_TEMPLATE = {
  ...
  triceps: (v) => [
    sx("Single-Arm Cross-Body Extension", 20, [12, 15, 20], 2, "7–8", "ohtri", "crossbody", false, "The hard triceps work was Saturday — keep this light", 5, v),
    sx("Cable Preacher Curl", 40, [10, 12, 15], 3, "8–9", "curl", "inccurl", false, "Antagonist balance", 5, v),
...
function saturdaySession(wave, week, spec) {
  ...
  if (triSpec) {
    blocks.push(sx("Overhead Cable Extension", 50, [10, 12, 15], reduced ? 2 : 3, "8–9", "ohtri", "ohrope", false, "Long-head bias — arms overhead", 5, wave));
    if (!reduced) blocks.push(sx("Rope / Single-Arm Pushdown", 50, [12, 15, 20], 2, "8–9", "pushdown", "pushdown", false, "", 5, wave));
  } else {
    blocks.push(sx("Overhead Cable Extension", 50, [10, 12, 15], reduced ? 2 : 3, "8–9", "ohtri", "ohrope", false, "Triceps slot", 5, wave));
  }
...
function sundaySession(wave, week, spec) {
  ...
  const tmpl = isDefaultSpec(spec) ? DETAIL_TEMPLATE.recommended(wave) : (DETAIL_TEMPLATE[spec.detail] || DETAIL_TEMPLATE.recommended)(wave);
  return tmpl;
}
